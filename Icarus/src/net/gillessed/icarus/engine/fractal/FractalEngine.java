...
public class FractalEngine extends AbstractEngine<BufferedImage> {
    ...
    public FractalEngine(FlameModel flameModel, int pixelWidth, int pixelHeight, ViewRectangle viewRectangle, int superSampleAmount) {
        ...
        int downSampleThreadSplit = DOWNSAMPLE_THREAD_SPLIT;
        if(downSampleThreadSplit > superSampleAmount) {
            downSampleThreadSplit = superSampleAmount;
        }
        int sdx = pixelWidth / downSampleThreadSplit;
        int sdy = pixelHeight / downSampleThreadSplit;
        for(int x = 0; x < downSampleThreadSplit; x++) {
            for(int y = 0; y < downSampleThreadSplit; y++) {
                int ex = (x + 1) * sdx;
                if(x == downSampleThreadSplit - 1) {
                    ex = pixelWidth;
                }

                int ey = (y + 1) * sdy;
                if(y == downSampleThreadSplit - 1) {
                    ey = pixelHeight;
                }
